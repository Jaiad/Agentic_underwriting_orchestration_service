...
from .mongodb_client import get_collection, get_mongodb_client
try:
    import mongomock
except ImportError:
    mongomock = None
...
class VectorSearchService:
    ...
    def search(
        ...
        try:
            ...
            if mongomock and isinstance(client, mongomock.MongoClient):
                # Return dummy results for mock execution
                return [{
                    "content": f"Mock content for {query}",
                    "score": 0.95,
                    "bic_code": "0044",  # Mock BIC for construction
            ...
            print(f"Warning: Vector search failed (likely no MongoDB connection): {e}")
            return []
